...
export class UpdateEmployeeUseCase {
  ...
  async execute(companyId: string, id: string, dto: UpdateEmployeeDto): Promise<ResponseEmployeeDto> {
    const employee = await this.updateEmployeeService.execute(companyId, id, dto);
    return {
      id: employee.id,
